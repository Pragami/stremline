...
const registerUser = asyncHandler(async (req, res) => {
  const { name, email, password, isAdmin, role, title, department } = req.body;
...
const getTeamList = asyncHandler(async (req, res) => {
  ...
  const user = await User.find()
    .select("name title role email isActive department")
...
const getDepartmentStaff = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const staffs = await User.find({ department: id }).select(
