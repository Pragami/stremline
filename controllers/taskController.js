...
const createTask = asyncHandler(async (req, res) => {
  try {
    ...
    const task = await Task.create({
      ...
      activities: activity,
      department,
      isApproved: false, // ✅ New field to track admin approval
    });
...
// ✅ Users request task completion (Admin must approve)
const requestTaskCompletion = asyncHandler(async (req, res) => {
  try {
    ...
    if (task.stage !== "in progress") {
      return res.status(400).json({ status: false, message: "Task must be 'in progress' before requesting completion." });
    }
...
const approveTaskCompletion = asyncHandler(async (req, res) => {
  try {
    const { id } = req.params;
    const task = await Task.findById(id);
    ...
    res.status(200).json({ status: true, message: "Task marked as completed." });
  } catch (error) {
    res.status(400).json({ status: false, message: error.message });
  }
});
...
const updateTask = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { title, date, team, stage, priority, assets, department } = req.body;
...
export {
  ...
  updateTask,
  updateTaskStage,
  requestTaskCompletion, // ✅ New function for users to request completion
  approveTaskCompletion, // ✅ New function for admin to approve completion
};